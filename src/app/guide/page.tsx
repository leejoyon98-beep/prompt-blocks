...
const concepts = [
  ...
  },
  {
    ...
    examples: ["ELI5", "TL;DR", "table", "redteam"],
  },
  {
    title: "블록팩",
...
const steps = [
  {
    title: "블록팩을 만들어요",
    description: "직접 만들거나 추천 블록팩으로 시작해요.",
  },
  {
    title: "필요한 블록과 태그를 담아요",
    description: "자주 쓰는 작업 방식과 답변 방식을 골라요.",
  },
  {
    title: "사용하기를 눌러요",
  ...
  },
  {
    ...
    description: "ChatGPT, Claude, Gemini 같은 AI에 붙여넣어 사용할 수 있어요.",
  },
  {
    title: "짧은 조합으로 요청해요",
    description: "이후에는 '핵심요약 + ELI5 + table'처럼 짧게 입력해도 돼요.",
  },
];
...
const benefits = [
  {
    ...
    description: "자주 쓰는 요청 방식을 매번 다시 쓰지 않아도 돼요.",
  },
  {
  ...
  },
  {
    ...
    description: "고객분석 + 구매장벽분석 + benefits처럼 이어 붙여 사용할 수 있어요.",
  },
  {
    title: "AI 도구가 바뀌어도 활용할 수 있어요",
    description: "복사한 등록 문구를 붙여넣는 방식이라 여러 AI에서 비슷하게 쓸 수 있어요.",
  },
];
  ...
  return (
    <PageShell className="py-14">
      <header className="border-b border-border pb-16">
        ...
        <h1 className="max-w-[680px] text-[36px] font-semibold leading-tight sm:text-[46px]">
          매번 같은 프롬프트를 다시 쓰지 않도록
        </h1>
        <p className="mt-5 max-w-[680px] text-[15px] leading-relaxed text-muted">
          프롬프트 블록은 자주 쓰는 AI 요청 방식을 짧은 블록으로 저장해두는 방법이에요.
          블록팩으로 묶어 내가 사용하는 AI에 붙여넣어두면, 이후에는 짧은 조합만으로 반복 작업을 빠르게 요청할 수 있어요.
        </p>
      </header>
      ...
      <main className="space-y-24 py-20">
        <section className="max-w-[820px]">
          <SectionHeading
            eyebrow="왜 필요한가요?"
            title="AI를 쓰다 보면 같은 말을 계속 반복하게 돼요."
          />
          <div className="mt-8 flex max-w-[720px] flex-wrap gap-2.5">
            {repeatedRequests.map((request) => (
              <span
                key={request}
                className="rounded-full border border-border bg-background px-4 py-2 text-[13px] text-foreground"
              >
                {request}
              </span>
            ))}
          </div>
          <p className="mt-8 max-w-[640px] text-[15px] leading-relaxed text-muted">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            {concepts.map((concept) => (
              <article key={concept.title} className="rounded-[var(--radius-card)] border border-border bg-background p-5">
                ...
                <p className="mt-3 text-[14px] leading-relaxed text-muted">{concept.description}</p>
          ...
          <div className="mt-10 space-y-0">
            {steps.map((step, index) => (
              <TimelineStep key={step.title} index={index + 1} isLast={index === steps.length - 1}>
                ...
                <p className="mt-2 max-w-[620px] text-[14px] leading-relaxed text-muted">{step.description}</p>
              </TimelineStep>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-[900px]">
          <div className="rounded-[var(--radius-card)] border border-border bg-subtle p-6 sm:p-8">
            <h2 className="text-[24px] font-semibold">꼭 알아두세요.</h2>
            <p className="mt-5 max-w-[720px] text-[14px] leading-relaxed text-foreground">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2">
            {useExamples.map((example) => (
              <article key={example.command} className="rounded-[var(--radius-card)] border border-border bg-background p-5">
                <p className="font-mono text-[13px] font-medium leading-relaxed text-foreground">{example.command}</p>
                <p className="mt-3 text-[13px] leading-relaxed text-muted">{example.request}</p>
              </article>
            ))}
          </div>
        </section>
      ...
      <section className="rounded-[var(--radius-card)] border border-border bg-background p-6 sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-[24px] font-semibold">자주 쓰는 요청부터 블록팩으로 만들어보세요.</h2>
...
function SectionHeading({
  ...
  return (
    <div>
      ...
      <h2 className="mt-3 max-w-[720px] text-[28px] font-semibold leading-tight sm:text-[34px]">{title}</h2>
      {description ? <p className="mt-4 max-w-[640px] text-[14px] leading-relaxed text-muted">{description}</p> : null}
    </div>
  );
}
